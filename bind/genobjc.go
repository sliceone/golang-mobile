...
func (g *ObjcGen) GenH() error {
	...
	if len(g.vars) > 0 {
		...
		for _, obj := range g.vars {
			...
			g.objcdoc(g.docs[obj.Name()].Doc())
			name := objcNameReplacer(lowerFirst(obj.Name()))
			g.Printf("+ (%s) %s;\n", objcType, name)
			g.Printf("+ (void) set%s:(%s)v;\n", initialUpper(name), objcType)
			g.Printf("\n")
		}
		g.Printf("@end\n\n")
	}
...
func (g *ObjcGen) genVarM(o *types.Var) {
	...
	// setter
	g.Printf("+ (void) set%s:(%s)v {\n", initialUpper(objcNameReplacer(lowerFirst(o.Name()))), objcType)
	g.Indent()
...
func (g *ObjcGen) genSetter(oName string, f *types.Var) {
	...
	g.Printf("- (void)set%s:(%s)v {\n", initialUpper(objcNameReplacer(lowerFirst(f.Name()))), g.objcType(t))
	g.Indent()
	g.Printf("int32_t refnum = go_seq_go_to_refnum(self._ref);\n")
